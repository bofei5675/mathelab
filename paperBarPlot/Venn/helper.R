finderPathways <- function(SourceId,rampIdHasPathway,haspathway = T){
  ...
  #return(df)
  if(haspathway){
    rampId <- unique(intersect(df$rampId,rampIdHasPathway))
  } else {
    rampId <- unique(df$rampId)
  } # end if find rampId with pathway if true, find all ramp
